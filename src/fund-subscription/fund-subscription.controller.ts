...
export class FundSubscriptionController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Get all subscriptions by Investor',
    description:
      'This endpoint is for investors to retrieve their own subscriptions.',
  })
  @ApiResponse({
    status: 200,
    description: 'List of subscriptions for the logged-in investor',
  })
  @ApiResponse({
    status: 403,
    description: 'Forbidden. Only accessible by authenticated investors.',
  })
  @Roles(RoleEnum.user)
  @Get()
  async getUserSubscriptions(
    @Req() req: Request,
  ): Promise<Subscription[] | { message: string }> {
  ...
  async getSubscriptionsByFundIdForInvestor(
    ...
    const investorId = (req.user as User).id;
    const subscriptions =
      await this.subscriptionService.getSubscriptionsByFundIdForInvestor(
        fundId,
        investorId,
      );
...
export class FundSubscriptionByManagerController {
  ...
  @ApiOperation({
    summary: 'For Manager to Approve a Subscription',
    description:
      'This endpoint allows a manager to approve a specific subscription.',
  })
  @ApiResponse({
    status: 200,
    description: 'The subscription has been successfully approved.',
  })
  @ApiResponse({
    status: 403,
    description: 'Forbidden. Only accessible by managers.',
  })
  ...
  @Roles(RoleEnum.manager)
  @ApiOperation({
    summary: 'For Manager to Reject a Subscription',
    description:
      'This endpoint allows a manager to reject a specific subscription.',
  })
  @ApiResponse({
    status: 200,
    description: 'The subscription has been successfully rejected.',
  })
  @ApiResponse({
    status: 403,
    description: 'Forbidden. Only accessible by managers.',
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'For managers to get all subscriptions based on their fund(s) ',
    description: 'This endpoint is for retrieving all subscriptions.',
  })
